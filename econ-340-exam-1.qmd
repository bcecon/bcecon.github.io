...
**Explanation**:  
**Deadweight loss (DWL)** represents the **lost welfare** due to trades that **could have benefited both buyers and sellers** but do not take place because of some distortion (such as taxes, externalities, or price controls). The **government collecting revenue is not inherently a welfare loss**, and **price changes alone do not create DWL** unless they **prevent mutually beneficial trades** from occurring.
...
A **positive externality** is a **positive side effect** of a market transaction that **benefits others who are not directly involved** as buyers or sellers. As a result, the **social marginal benefit ($SMB$)** is **greater** than the **private marginal benefit ($MB$)**.
...
A public good is a good that is non-excludable (people cannot be prevented from using it) and non-rivalrous (one person’s consumption does not reduce availability for others). Examples include clean air, climate stability, or lighthouse signals.
...
::: {.callout-tip collapse="true"}
#### Show answer
In **environmental economics**, many **public goods generate positive externalities** because individuals or communities who contribute to their provision **create benefits that extend beyond the contributor**. For example, when one region **preserves a forest**, the **carbon sequestration benefits** are enjoyed globally. However, since these benefits are **not priced or captured in private markets**, the incentive to invest in conservation is **below the socially optimal level**. This leads to the **under-provision of environmental public goods** such as **clean air**, **biodiversity**, and **climate stability**.
...
Community governance can enhance PES success by enabling local monitoring, social pressure, and rule enforcement, which reduce illegal harvesting. When communities collectively agree on conservation rules and share PES payments, peer accountability increases compliance. However, community governance can also create challenges if elite capture, unequal distribution of benefits, or lack of trust arises. If some members believe others will free-ride or illegally harvest while still sharing the PES payment, they may be discouraged from cooperating. This tension highlights the dual role of community governance as both a solution through cooperation and a potential source of conflict and enforcement difficulty.
...
p_equil + 
  ...
  annotate("text", x = Q_M*0.45, y = 45, label = "TB", fontface = "bold")
```
...
p_equil +
  ...
  annotate("text", x = Q_M*0.65, y = 18, label = "TC", fontface = "bold")
```
...
p_equil +
  ...
  annotate("segment", x = 0, xend = Q_M, y = P_M, yend = P_M, linetype = "dashed") +
  annotate("text", x = Q_M*0.65, y = P_M - 8, label = "PS", fontface = "bold")
```
...
p_equil +
  ...
  annotate("text", x = Q_M*0.65, y = 28, label = "EC", fontface = "bold")
```
...
p_equil +
  geom_ribbon(data = df_DWL, aes(x = Q, ymin = MB, ymax = SMC),
              fill = "white", alpha = 0.0, color = "black", linetype = "dashed") +
  annotate("text", x = (Q_star + Q_M)/2, y = 42, label = "DWL", fontface = "bold")
```
