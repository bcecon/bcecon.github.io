...
<div style="display:block; margin:-15px;"></div>
  ...
  $$
  U(C_{t}) = \frac{C_{t}^{1-\eta} - 1}{1 - \eta} \qquad\text{and}\qquad    C_t + I_t = Y_t
  $$
<div style="display:block; margin:-15px;"></div>

- **Resource Allocation Across Generations**: In each period $t$, total output ($Y_t$) can be used for **consumption** or **investment**:   
...
- **Sustainability**: To maximize **overall well-being across generations**, **dynamic efficiency** seeks a path that **balances** present and future welfare:
  - <u>How much of today’s output should we **consume now** versus **invest for the future** (e.g., in renewable energy or infrastructure)?</u>
...
## Linking $\eta$, Diminishing Utility, and Fairness
...
- **Higher $\eta$ → Stronger Concern for Equality**  
  <!-- - Since extra consumption benefits the poor **more than** the rich, when resources are allocated over time, society with a higher $\eta$ places **more weight on poorer or earlier generations.** -->
  <!-- - Since an additional unit of consumption increases well-being **more for the poor than for the rich**, a society with a higher $\eta$ places **greater weight on poorer or earlier generations** when allocating consumptions over time. -->
  - Because extra consumption benefits the poor **more than** the rich, a higher $\eta$ means society places **greater weight on poorer or earlier generations** when choosing how to allocate resources ($C_{t}$ and $I_{t}$) over time.
