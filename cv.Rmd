...
cv_entries %>% 
  ...
  mutate(what = str_c("<span class='cv-entry-title' style = 'font-weight:bold'> <u>", what, "</u>&nbsp;</span><br> ", additional_info)) %>% 
  select(type, what) %>% 
  mutate(type = "") %>% 
  kbl(padding = 0, col.names = NULL, escape=FALSE) %>%
  kable_styling( bootstrap_options = c("hover", "condensed")) 
...
cv_entries %>% 
  ...
  mutate(what = ifelse(!is.na(additional_info), 
    str_c("<span class='cv-entry-title' style = 'font-weight:bold'> <i>", what, "</i>&nbsp; </span><br class='work-break'>", where, "<br>", additional_info),
