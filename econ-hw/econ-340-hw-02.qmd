...
df_bot <- tibble(L, MB, MC, MD, AB, AC) |>
  pivot_longer(-L, names_to = "curve", values_to = "value")
```
...
ggplot(df_top, aes(L, value, color = curve)) +
  ...
  annotate("text", x = L_priv, y = max(df_top$value)*0.9, label = "L_priv", vjust = -0.3) +
  annotate("text", x = L_OA,   y = max(df_top$value)*0.85, label = "L_OA", vjust = -0.3) +
...
ggplot(df_bot, aes(L, value, color = curve)) +
  ...
  annotate("text", x = L_priv, y = max(df_bot$value, na.rm = TRUE)*0.90, label = "L_priv", vjust = -0.3) +
  annotate("text", x = L_OA,   y = max(df_bot$value, na.rm = TRUE)*0.85, label = "L_OA", vjust = -0.3) +
