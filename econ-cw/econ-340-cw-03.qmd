...
**Why?** At $h=1$, the next unit’s benefit equals the next unit’s cost, so total surplus (the “pie”) is maximized:  
$$
TS(h)= TB(h) - TC(h),
$$  
which peaks at $h=1$.
...
- Person 1 should also be better off from the bargaining. 
  - This means that Person 1’s net payoff (the benefit from music plus any transfer received or minus any transfer paid) at $h=1$ with $T$ must be at least as high as at the initial entitlement $h=2$ with $T=0$.
...
## Why transfers don’t justify stopping at 0.5 or 1.5?

- The **total pie** from bargaining is the **total surplus**: the difference between Person 1’s total benefit from music and Person 2’s total harm.  

$$
TS(h) = TB(h) - TC(h)
$$

- Transfers ($T$) only redistribute this surplus between the two people; they don’t create or destroy it.  
