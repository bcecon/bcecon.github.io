...
- **Use of Revenue:**
	-	Revenue Recycling: The way carbon pricing revenue is used critically affects the policy’s equity. Options include:
...
For (Reduce, Reduce) to be only NE, the following conditions should hold:

1. Country A's best response to Country B's Reduce is Reduce:

$$
\begin{align}
\overbrace{5}^{\text{Payoff from Reduce}} & > \overbrace{8 -c}^{\text{Payoff from BAU}}\\
\quad\\
c & > 3\tag{1}
\end{align}
$$
...
$$
\begin{align}
\overbrace{-2+b}^{\text{Payoff from Reduce}} &> \overbrace{1 -c}^{\text{Payoff from BAU}}\\
\quad\\
b + c &> 3\tag{2}
\end{align}
$$
...
- **Situation Where Issue Linkage Fails to Satisfy Conditions from Part (a):**
  - **Scenario:** Weak Environmental Aid Reduces $b$ Below Required Threshold
	...
	-	**Consequence:** Country A (and similarly Country B) may prefer “BAU” when the other country chooses “BAU”, leading to multiple Nash equilibria, including (BAU, BAU), or a unique NE of (BAU, BAU).
...
-	The failure to meet the condition $b + c > 3$ means “Reduce” is not a *dominant* strategy, and (Reduce, Reduce) is no longer the only Nash equilibrium.
